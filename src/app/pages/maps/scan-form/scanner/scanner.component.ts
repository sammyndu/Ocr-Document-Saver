...
export class ScannerComponent implements OnInit {
  ...
  toggleStartDemo() {
    this.dwtService.bUseService = true;
    this.dwtService.bUseCameraViaDirectShow = this.bUseCameraViaDirectShow && true;
  }
  ngOnInit() {
    this.toggleStartDemo();
    this.eventsSubscription = this.eventsSubject.asObservable().subscribe(
      (args: any) => {
        if (args.type)
          switch (args.type) {
            case "resize":
  ...
  doOCR() {
    this.ocrEngineChange("Pro");
    this.ocrProOptions.FindTextAction = 0;
    this.ocrProOptions.FindTextFlags = 1;
    this.ocrProOptions.Language = 'eng';
    this.ocrProOptions.OutputFormat = 'TXTS';
    this.ocrProOptions.PDFAVersion = 'pdf/a-1a'
    this.ocrProOptions.PDFVersion = '1.5';
    this.ocrProOptions.RecognitionModule = 'AUTO';
    this.ocrProOptions.bFindText = false;
    this.ocrProOptions.engine = 'pro';
    this.ocrProOptions.textToFind = 'TWAIN';
        ...
        res => {
          ...
          switch (format) {
            ...
            case "0" /* TEXT */:
              ...
              if(this.ocrResultString.match(nameRe) != null) {
                console.log(this.ocrResultString.match(nameRe)[0]);
                this.documentName = this.ocrResultString.match(nameRe)[0];
              } else {
                this.documentName = "";
              }
